...
export default function PreMatchTeam({ teamName, side, isMyTeam }: PreMatchTeamProps) {
  return (
    <div
      className={cn("w-full relative   h-full min-w-[90px] min-h-[300px] ")}
    >
      <div
        className="absolute inset-0 bg-contain bg-no-repeat bg-center z-0 w-full h-full "
        style={{
          backgroundImage: isMyTeam ? "url('/pre-match/myTeamContainer.svg')" : "url('/pre-match/enemyTeamContainer.svg')",
        }}
      />
      ...
      >
        <div className="w-full h-full flex items-center justify-center max-w-[60px] max-h-[60px]">
          <img src={"/teams/dojoUnited.png"} alt="team" className="w-full h-full object-cover" />
        </div>
        <h1 className={cn(
