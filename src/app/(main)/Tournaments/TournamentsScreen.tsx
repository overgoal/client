        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {tournaments.map((tournament) => (
              <Card key={tournament.id} className="bg-slate-800/50 border-slate-700 p-6">
                <div className="flex flex-col h-full">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      <h2 className="text-2xl font-semibold text-white mb-2">{tournament.name}</h2>
                      <div className="flex items-center space-x-2 mb-3">
                        <span className={`px-2 py-1 rounded-full text-white text-xs font-medium ${getTypeColor(tournament.type)}`}>
                          {tournament.type}
                        </span>
                        <span className={`px-2 py-1 rounded-full text-white text-xs font-medium ${getStatusColor(tournament.status)}`}>
                  ...
                  <p className="text-slate-300 mb-4 flex-grow">{tournament.description}</p>
                  ...
                  <div className="space-y-2 mb-6">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-white">{tournament.currentRound}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-400">Duration</span>
                      <span className="text-white">{tournament.startDate} - {tournament.endDate}</span>
                    </div>
                  </div>
